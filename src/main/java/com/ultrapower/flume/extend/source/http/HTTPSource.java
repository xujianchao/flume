 ...
 */
package com.ultrapower.flume.extend.source.http;
...
public class HTTPSource extends AbstractSource implements EventDrivenSource, Configurable {
	 ...
	 * connectors in each source which binds to the port defined for that source. b.
	 * Each source starts its own server instance, which binds to the source's port.
	 *
	 * b is more efficient than a because Jetty does not allow binding a servlet to
	 * a connector. So each request will need to go through each each of the
...
//			List<Event> eventsFlag = Collections.emptyList();
			List<Event> eventsFlag = new ArrayList<Event>();
			try {
				events = handler.getEvents(request);
				// 给msg打标签
				for (Event e : events) {
					Event eflag = addFlag(e);
					eventsFlag.add(eflag);
				}
			...
			sourceCounter.addToEventReceivedCount(eventsFlag.size());
			try {
				getChannelProcessor().processEventBatch(eventsFlag);
			...
			sourceCounter.addToEventAcceptedCount(eventsFlag.size());
		}
	 ...
	 * @param event
	 */
	private Event addFlag(Event event) {
		try {
			JSONObject json = JSONObject.parseObject(new String(event.getBody()));
			String tag = json.getString("tag");
			if (tag != null && "monitor".equals(tag)) {
				JSONArray dataIp = json.getJSONArray("dataip")==null?new JSONArray():json.getJSONArray("dataip");
				JSONArray dataPathTime = json.getJSONArray("datapathtime")==null?new JSONArray():json.getJSONArray("datapathtime");
				JSONArray datapath = json.getJSONArray("datapath")==null?new JSONArray():json.getJSONArray("datapath");
//				InetAddress addr = InetAddress.getLocalHost();
//				String ip = addr.getHostAddress().toString(); // 获取本机ip
				dataIp.add(local_ip);
				dataPathTime.add(System.currentTimeMillis());
				datapath.add("flume-server-monitor");
		   ...
		   LOG.error("给日志打标签出错"+e.getMessage());
		}
		return event;
